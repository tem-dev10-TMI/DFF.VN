...
<div class="<?= $is_mobile ? '' : 'block-k left-menu bg-transparent border0 p-l-0' ?>">
    ...
    <?php if (!empty($moreTopics)): ?>
        ...
        <div class="to-expend">
            <?php if ($is_mobile): ?>
                <i class="accordion-button collapsed" data-bs-toggle="collapse" data-bs-target="#<?= $collapse_id ?>" aria-expanded="false" aria-controls="<?= $collapse_id ?>"></i>
            <?php else: ?>
                <button class="accordion-button collapsed" data-bs-toggle="collapse" data-bs-target="#<?= $collapse_id ?>" aria-expanded="true" aria-controls="<?= $collapse_id ?>"></button>
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    <ul class="<?= $is_mobile ? 'about-c' : 'nav nav-second-level about-c' ?>">
        ...
        <li <?= $is_mobile ? '' : 'class="mline"' ?>>
            ...
            <a href="about#gioithieu"> <?= $is_mobile ? 'Chính sách nội dung' : 'Thỏa thuận cung cấp và sử dụng dịch vụ MXH' ?></a>
        </li>
        <li>
            <svg rpl="" fill="currentColor" height="20" icon-name="topic-law-outline" viewBox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
